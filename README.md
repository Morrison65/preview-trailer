...
To update the installed Shortkeys extension from GitHub, open the Shortkeys options page, open its DevTools console, and paste [update-shortkeys-from-github.js](update-shortkeys-from-github.js). It fetches the repository export, normalizes `id`, `enabled`, `sites`, and `sitesArray` like Shortkeys, deletes and verifies all existing sync/local shortcut keys, then writes the replacement `keys` value using chunked `storage.sync` with a `storage.local` backup and local fallback. It stores bounded structured logs in localStorage and, after a successful replacement, downloads them as `Logs/preview-trailer-update-<timestamp>.json` without prompting when the extension downloads API is available. Logs are cleared only after download initiation succeeds. Browsers do not permit a page script to silently write to an absolute Windows path; the file is saved under the browser's configured Downloads directory (or its `Logs` subdirectory when supported). The script automatically runs once when pasted and also exposes `updateShortkeysFromGithub()` for another run. It cannot update extension storage from an ordinary website or Node process.
